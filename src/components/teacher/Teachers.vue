<template>
    <div>
        ...
        <el-card>
            ...
            <el-row :gutter="20">
                <el-col :span="8">
                    <!-- 搜索与添加 -->
                    <el-input placeholder="请输入内容" v-model="query.query" clearable @clear="getTeacherList">
        ...
        <!-- 教师列表区域 -->
        <el-table :data="teacherlist" border stripe :header-cell-style="headClass">
            ...
            <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                    <el-tooltip effect="dark" content="编辑教师信息" placement="top-start" :enterable="false">
  ...
  <el-form-item label="教师姓名">
    <el-input v-model="editForm.teacherName" disabled></el-input>
  </el-form-item>
  <el-form-item label="邮箱" prop="email">
...
export default {
    ...
    methods: {
        ...
        async search() {
            const { data : res} = await this.$http.get('teachersearch',{ params: this.query})
            if(res.code !== 200) {
        ...
        editTeacherInfo() {
            this.$refs.editFormRef.validate(async valid=> {
                if(!valid) return
                //发起修改教师信息的数据请求
                const {data: res} = await this.$http.put('teacher/' + this.editForm.id,{email:
        ...
        async removeTeacherById(id) {
            //弹框询问  是否删除数据
            const confirmResult = await this.$confirm('此操作将永久删除该教师信息, 是否继续?', '提示', {
        ...
        //如果教师确认删除，则返回字符串 confirm
        //如果教师取消删除，则返回字符串 cancel
