  ...
  export default {
    data() {
      return {
        ws: null,
        count: 0,
        userId: this.$store.getters.getUser.id, // 当前用户ID
        username: this.$store.getters.getUser.name, // 当前用户昵称
        avatar: this.$store.getters.getUser.avatar, // 当前用户头像
        list: [], // 聊天记录的数组
    ...
    methods: {
      ...
      initWebSocket() {
        ...
        if (window.WebSocket) {
          ...
          var url = 'ws://' + serverHot + ':8082' + '/groupChat/' + sip + '/' + this.userId; // `ws://127.0.0.1/9101/groupChat/10086/聊天室`
