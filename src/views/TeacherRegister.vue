<template>
  ...
  <el-form :model="registerForm" :rules="rules" class="login-container" label-position="left"
    ...
    <h3 class="login_title">教师注册</h3>
    <el-form-item prop="teacher_name">
      <el-input type="text" v-model="registerForm.teacher_name"
...
export default{
  ...
  data () {
    return {
      rules: {
        teacher_name: [{required: true, message: '用户名不能为空', trigger: 'blur'},
  ...
  methods: {
    register () {
      ...
      this.axios.post("/teacher/register",{
            teacher_name:this.registerForm.teacher_name,
...
<style>
  #paper {
    background:url("../assets/img3.webp") no-repeat;
    background-position: center;
    background-repeat: 'repeat';
    background-size: '45vh 15vw';
    height: 100%;
    width: 100%;
